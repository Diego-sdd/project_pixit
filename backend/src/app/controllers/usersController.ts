...
class Users {
  async login (request, response) {
    ...
    try {
      const resultRegisterUser = await UsersModel.getUserRegister(email)
  ...
  async index (request, response) {
    ...
    try {
      const [checkUserEmail] = await UsersModel.getUserRegister(email)
      ...
      const resultInsert = await UsersModel.insertUser(email,
        hashPassword, name)
